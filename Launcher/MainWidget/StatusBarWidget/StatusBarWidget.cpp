...
static void initializeRunnableCallback(void *paramater)
{
    ...
    StatusBarWidgetPrivate *ptr = static_cast<StatusBarWidgetPrivate *>(paramater);
    if ((NULL != ptr)
            && (NULL == ptr->m_Background)) {
        QFile file(":/Images/Resources/Images/StatusBarWidgetBackground.dat");
        if (file.open(QIODevice::ReadOnly)) {
            QDataStream stream(&file);
            stream >> ptr->m_ByteArray;
            file.close();
            ptr->m_Background = new QImage((const uchar *)ptr->m_ByteArray.data(), 1280, 60, QImage::Format_RGB32);
        }
    }
}
...
//        if (UserInterfaceUtility::loadFromdata(QString(":/Images/Resources/Images/StatusBarWidgetBackground.dat"), m_ByteArray)) {
//            m_Background = new QImage(StatusBarWidgetBackground, 1280, 60, QImage::Format_RGB32);
            m_Background = new QImage(QString(":/Images/Resources/Images/StatusBarWidgetBackground.png"));
//        }
//        QFile file(":/Images/Resources/Images/StatusBarWidgetBackground.dat");
//        if (file.open(QIODevice::ReadOnly)) {
//            QDataStream stream(&file);
...
//    initializeRunnableCallback(this);
